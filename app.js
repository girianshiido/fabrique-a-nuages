...
const els = {
  ...
  ,venusSystems:$("#venusSystems"),overdriveButton:$("#overdriveButton"),corrosionLabel:$("#corrosionLabel"),corrosionFill:$("#corrosionFill"),overdriveHint:$("#overdriveHint"),venusConstructionProgress:$("#venusConstructionProgress"),venusConstructionList:$("#venusConstructionList")
};
...
function updateVenusSystems(delta){
  ...
  if(state.venusOverdrive){state.venusCorrosion=Math.min(100,state.venusCorrosion+8*delta);if(state.venusCorrosion>=100){state.venusOverdrive=false;state.venusCooldownUntil=now()+30000;toast("Corrosion critique — surcadence verrouillée 30 secondes !");playTone(180,.3)}}
...
function renderVenusSystems(){
  ...
  const cooling=Math.max(0,(state.venusCooldownUntil-now())/1000);els.corrosionLabel.textContent=`${state.venusCorrosion.toFixed(0)} %`;els.corrosionFill.style.width=`${state.venusCorrosion}%`;
  ...
  els.overdriveHint.textContent=state.venusOverdrive?"Surcadence active : production ×25, corrosion +8 %/s.":cooling>0?"Refroidissement forcé des structures flottantes.":"Produit 25× plus vite pendant environ 12 secondes.";
